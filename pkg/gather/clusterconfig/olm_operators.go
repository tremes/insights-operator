...
func gatherOLMOperators(ctx context.Context, dynamicClient dynamic.Interface) ([]record.Record, []error) {
	...
	csvs, err := dynamicClient.Resource(gvr).List(ctx, metav1.ListOptions{})
	...
	for _, csv := range csvs.Items {
		var version string
		err := parseJSONQuery(csv.Object, "spec.version", &version)
		if err != nil {
			klog.Errorf("Cannot find \"spec.version\" in %s definition: %v", csv.GetName(), err)
			continue
		}
		olmO := olmOperator{
			Name:    csv.GetName(),
